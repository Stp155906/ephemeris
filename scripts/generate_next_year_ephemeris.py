...
PLANET_MAP = {"SU":"Sun","MO":"Moon","ME":"Mercury","VE":"Venus","MA":"Mars","JU":"Jupiter","SA":"Saturn","UR":"Uranus","NE":"Neptune","PL":"Pluto"}
ZODIAC_MAP = {"ARI":"Aries","TAU":"Taurus","GEM":"Gemini","CAN":"Cancer","LEO":"Leo","VIR":"Virgo","LIB":"Libra","SCO":"Scorpio","SAG":"Sagittarius","CAP":"Capricorn","AQU":"Aquarius","PIS":"Pisces"}
...
def extract_day_num(tr):
    ...
    import re
    m = re.search(r"\b([0-3]?\d)\b", tds[0].get_text(" ", strip=True))
...
def fetch_month(month, year):
    ...
    for attempt in range(3):
        try:
            ...
            if not table: 
                print(f"  ⚠️ Table not found for {month_name} {year}"); return None
            break
        except Exception as e:
            if attempt == 2: 
                print(f"  ❌ Failed {month_name} {year}: {e}"); return None
            time.sleep(1.5*(attempt+1))
    month_data = {}
    for tr in table.find_all("tr"):
        planet_tds = tr.find_all("td", class_="udaj_planeta")
        if len(planet_tds) < 10: continue
        day_num = extract_day_num(tr)
        if day_num is None: continue
        daily = {}
        for abbr, td in zip(PLANET_ABBRS, planet_tds[:len(PLANET_ABBRS)]):
            img = td.find("img"); sign_abbr = img["alt"].strip() if img and img.has_attr("alt") else None
            sign = ZODIAC_MAP.get(sign_abbr, "Unknown")
            spans = td.find_all("span"); degree = " ".join(s.get_text(strip=True) for s in spans)
            daily[PLANET_MAP[abbr]] = f"{sign} {degree}".strip()
        month_data[str(day_num)] = daily
    if not month_data: 
        print(f"  ⚠️ No rows parsed for {month_name} {year}"); return None
...
def build_year(year):
    year_data = {}; print(f"📅 Building year: {year}")
    for m in range(1, 13):
        print(f"  🔄 {calendar.month_name[m]} {year} ...")
        r = fetch_month(m, year)
        if not r: continue
        mn, md = r; year_data[mn] = md; time.sleep(1.0)
    return year_data
...
def main():
    ...
    if not data: raise SystemExit(\"No data generated; aborting.\")
